...
public class BlogPostImpl implements BlogPostDAO {
	...
	public List<BlogPost> getBlogs(int approved) {
		...
		// if approved = 0 , select * from blogpost where approved =0 and
		// rejection reason is null; blogs waiting for approval
		// if approved =1, select * from blogpost where approved = 1; blogs
		// waiting for approved
		String queryStr = "";
		if (approved == 1) // list of blogs approved
		{
			queryStr = "from BlogPost where approved=" + approved;
		} else // list of blogs waiting for approval [ingnore the blogpost which
				// are alreay reject by admin]
	...
	public List<BlogPost> getNotification(String username) {
		...
		Query query = session.createQuery(
				"from BlogPost where postedBy.username=? and viewed=? and (approved=1 or rejectionReason!=null)");
